...
shinyServer(function(input, output) {
  ...
  rating <- data.frame(matrix(NA, ncol = 2, nrow=6))
  rating[,1] <- c("1 out of 5","2 out of 5","3 out of 5", "4 out of 5","5 out of 5","average rating")
  rating[1,2] <- data_trust %>% filter(rating == 1) %>% nrow()  %>% round( digits =0)
  rating[2,2] <- data_trust %>% filter(rating == 2) %>% nrow()  %>% round( digits =0)
  rating[3,2] <- data_trust %>% filter(rating == 3) %>% nrow()  %>% round( digits =0)
  rating[4,2] <- data_trust %>% filter(rating == 4) %>% nrow()  %>% round( digits =0)
  rating[5,2] <- data_trust %>% filter(rating == 5) %>% nrow() %>% round( digits =0)
  rating[6,2] <- mean(as.numeric(data_trust$rating))%>%round( digits =2)
  
  
colnames(rating) <- c(" ", " value")
  ...
  tf_plot <- data_trust.freq %>% top_n(20, frequency) %>%
    ...
    xlab("Frequency") + ylab("term") +ggtitle("top 20 most frequent words") + scale_y_continuous(breaks = seq(0,max(data_trust.freq$frequency), by =1))
  ...
  data_trust.feat <- textstat_frequency(data_trust.tf) %>% filter(rank <= 40) 
